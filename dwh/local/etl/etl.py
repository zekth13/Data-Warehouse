...
def transform(choice, df):
    try:
        if choice == "suppliers":
            # df.drop_duplicates('SUP_NO')
            suppliers_df = pd.DataFrame(df)
            return suppliers_df
...
def user_choice(choice):
    ...
    if choice == "register":
        x = ["suppliers", "outlet", "product", "sales"]
...
# Prompt user
def prompt_user():
    option = ["[#ffff00]suppliers[/#ffff00]", "[#00cc00]outlets[/#00cc00]", "[#00ffff]products[/#00ffff]", "[#ff00ff]sales[/#ff00ff]", "type [#ff0000]register[/#ff0000] to register in Prefect Cloud"]
    print("\nETL Option:")
    for i in option:
        rich.print(" " + i)
    ...
    if (choice == "suppliers" or choice == "outlets" or choice == "products" or choice == "sales" or choice == "register"):
        user_choice(choice)
        prompt_user()
